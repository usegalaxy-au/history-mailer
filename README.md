...
The history mailer is a python script to manage histories on a [Galaxy](https://github.com/galaxyproject/galaxy) instance. It interacts with the Galaxy API and a mail API using admin privileges to detect unused histories, alert users on unused histories and delete these if the user does not update their histories. The history mailer maintains a local database to keep track of email notifications sent to users.
